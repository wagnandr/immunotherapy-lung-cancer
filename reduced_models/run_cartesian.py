...
def run(
    ...
    initial_guess = df.Expression(('1. / (exp( a * (sqrt(x[0]*x[0]+x[1]*x[1]+x[2]*x[2]) -r) ) + 1)', '0'), r=0.002, a=8000, degree=2)

    nutrients = df.Expression('1 + a * x[2]', a=100, degree=1)

    mesh = df.BoxMesh(df.Point(-x_max, -x_max, -x_max), df.Point(x_max, x_max, x_max), N, N, N)
...
if __name__ == '__main__':
    ...
    save_at = 'output_cartesian'

    run(N, x_max, final_time, dt, 7, params, start_therapy, length_therapy, save_at)